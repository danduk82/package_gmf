...
for (const filename of ls(path.resolve(__dirname, 'package_gmf_geoportal/static-ngeo/js/apps/*.html.ejs'))) {
  const name = filename.file.substr(0, filename.file.length - '.html.ejs'.length);
  entry[name] = 'package_gmf/apps/Controller' + name + '.js';
  plugins.push(
...
const projectRule = {
  test: /package_gmf_geoportal\/static-ngeo\/js\/.*\.js$/,
  use: {
    loader: 'babel-loader',
    options: {
...
module.exports = {
  ...
  resolve: {
    ...
    alias: {
      package_gmf: path.resolve(__dirname, 'package_gmf_geoportal/static-ngeo/js'),
    },
  },
};
