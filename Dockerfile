...
RUN \
    ...
    do \
        node /usr/bin/compile-catalog \
            ...
            /tmp/config/geoportal/package_gmf_geoportal/locale/${lang}/LC_MESSAGES/package_gmf_geoportal-client.po \
            > /tmp/config/geoportal/package_gmf_geoportal/static/${lang}.json; \
    done && \
    rm -rf /tmp/config/geoportal/package_gmf_geoportal/locale

RUN \
    cd /tmp/config/geoportal/ && \
    c2c-template --vars ${VARS_FILE} \
        --get-config package_gmf_geoportal/config.yaml \
        ${CONFIG_VARS} && \
    pykwalify --data-file package_gmf_geoportal/config.yaml \
